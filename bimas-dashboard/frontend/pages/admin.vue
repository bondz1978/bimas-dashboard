  ...
  <div class="flex flex-col min-h-screen" style="background: var(--cream)">
    ...
    <div v-if="!authenticated" class="flex-1 flex items-center justify-center p-10">
      <div class="w-full max-w-sm rounded-2xl border p-10 text-center" style="background: var(--white); border-color: var(--gray-200); box-shadow: var(--shadow-md)">
        ...
        <p class="text-sm mb-6" style="color: var(--text-muted)">Masukkan password untuk mengakses konfigurasi</p>
        <div class="flex flex-col gap-2.5">
          <input v-model="passwordInput" type="password"
            placeholder="Password admin..."
            class="w-full px-3.5 py-2.5 rounded-lg border text-sm outline-none transition-all"
            style="border-color: var(--gray-200); background: var(--gray-50); color: var(--text-primary); font-family: var(--font-body)"
            @keyup.enter="doLogin"
            @focus="$event.target.style.borderColor='var(--olive-500)'"
            @blur="$event.target.style.borderColor='var(--gray-200)'"
          />
          <div v-if="loginError" class="text-left text-xs font-medium" style="color: #dc2626">{{ loginError }}</div>
          <button @click="doLogin"
            class="w-full py-2.5 rounded-lg text-sm font-bold text-white cursor-pointer border-none"
            style="background: var(--olive-700)">Masuk</button>
        </div>
...
function doLogin() {
  const correct = (config.value || {}).admin_password || DEFAULTS.admin_password
